...
// Función para hacer la duración total en  hs mn seg
...
// para levantar con fethc

function discosEnJSON() {
  fetch('discos.json')

  //por si falla
    .then(response => {
      if (!response.ok) {
        throw new Error('Error al cargar los discos');
      }
    ...
    .then(data => {
      data.forEach(discoData => {
        ...
        let disco = new Disco(
          ...
          discoData.autor,
          discoData.codigo,
          discoData.imagenPortada
        );

        // Cargar las pistas en el dis
        discoData.pistas.forEach(pistaData => {
          let pista = new Pista(pistaData.nombre, pistaData.duracion);
          disco.agregarPista(pista);
        });

        // Agregar el disco a la bibloteca
        coleccionDiscos.push(disco);
      });

      // muestro el aviso de que esta ok
      alert('Los discos del JSON han sido cargados correctamente.');
    })
    .catch(error => {
      console.error('Error al cargar el  JSON:', error);
      alert('No se pudieron cargar los discos desde el JSON.');
    });
}
